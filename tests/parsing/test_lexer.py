...
class TestFindDot(unittest.TestCase):
    ...
    def test_find_dot(self):
        ...
        assignments = lexer._find_assignments(source)
        previous_dot = lexer._find_previous_dot(
            source, 0, assignments[0].start, assignments[0].previous_non_code_tokens
        )
        self.assertIsNone(previous_dot)
        next_dot = lexer._find_next_dot(
            source,
            assignments[0].end,
            len(source),
            assignments[0].previous_non_code_tokens,
        )
    ...
    def test_find_dot2(self):
        ...
        self.assertEqual(source[: next_dot.end()], "f(a) := 1.")
        previous_dot = lexer._find_previous_dot(
            source, 0, assignments[1].start, assignments[1].previous_non_code_tokens
        )
    ...
    def test_string(self):
        ...
        next_dot = lexer._find_next_dot(
            ...
            assignments[0].end,
            assignments[1].start,
            assignments[1].previous_non_code_tokens,
        )
        self.assertIsNotNone(next_dot)
        self.assertEqual(
            source[0 : next_dot.end()],
            'f("one string. something else.") := "another string. something else.".',
        )
        previous_dot = lexer._find_previous_dot(
            source, 0, assignments[1].start, assignments[1].previous_non_code_tokens
        ...
        next_dot = lexer._find_next_dot(
            ...
            assignments[1].end,
            len(source),
            assignments[2].previous_non_code_tokens,
        )
        self.assertIsNotNone(next_dot)
        self.assertEqual(
            source[previous_dot.end() + 1 : next_dot.end()],
            'g("yet another string. something else.") := "final string string. something else.".',
        )
...
class TestSplitCode(unittest.TestCase):
    def test_find_assignment_rules(self):
        ...
        self.assertIsInstance(rules[1], lexer.UnParsedAssignmentRule)
        self.assertIsInstance(rules[2], lexer.UnParsedAssignmentRule)
        self.assertIsInstance(rules[3], lexer.UnParsedAssignmentRule)
    ...
    def test_starting_clingo(self):
        ...
        self.assertIsInstance(rules[0], lexer.UnParsedClingoCode)
        self.assertIsInstance(rules[1], lexer.UnParsedAssignmentRule)
    ...
    def test_ending_clingo(self):
        ...
        self.assertEqual(len(rules), 2)
        self.assertIsInstance(rules[0], lexer.UnParsedAssignmentRule)
        self.assertIsInstance(rules[1], lexer.UnParsedClingoCode)
    ...
    def test_end_clingo(self):
        ...
        self.assertIsInstance(rules[0], lexer.UnParsedAssignmentRule)
        self.assertIsInstance(rules[1], lexer.UnParsedAssignmentRule)
        self.assertIsInstance(rules[2], lexer.UnParsedClingoCode)
        self.assertEqual(rules[0].source, "f(3) := 4.")
        self.assertEqual(rules[1].source, "\nf(5) := 6.")
        self.assertEqual(rules[2].source, "\np(a).\n   # comment\np(b) :- p(a).\n")
