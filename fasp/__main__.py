...
def check_versions(args) -> int:
    if sys.version_info < (3, 13):
        sys.stderr.write(f"*** ERROR: fasp requires Python 3.13 or higher, found version {sys.version_info} \n")
        return 1
    try:
        from clingo import core
        clingo_version = core.version()
    except ImportError:
        sys.stderr.write("*** ERROR: fasp requires clingo library version 6.0.0 or higher.\n")
        return 1
