...
from funasp.asp2funasp.util.types import CPredicate, FPredicate, FRelation
...
class FunctionalPredicateFinder:
    def __init__(self, library: Library) -> None:
        ...
        self.constraints: List[ast.Statement] = []
    ...
    def processProgram(
        self, statements: List[ast.Statement] | List[ast.StatementRule]
    ...
    def processFoundPredicates(self) -> List[FRelation]:
        ...
        for (name, arguments, arity), valuesList in groupedPredicates.items():
            ...
            if extractedValues == remainingIndices:
                self.functionalRelations.append(
                    FRelation(name, arity, arguments, valuesTuples)
                )
        return self.functionalRelations
