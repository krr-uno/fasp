...
import clingo_funasp.ast as ast
import clingo_funasp.core as core
from clingo_funasp.symbol import Number

import funasp.asp2funasp.util.util as util
from funasp.asp2funasp.transformers.preprocessing.base import PreprocessingTransformer


class ConstraintAggregateGuardTransformer(PreprocessingTransformer):
    """Normalize guards in constraint aggregates for clingo 6 ASTs."""
    ...
    def _(self, rule: ast.StatementRule) -> ast.StatementRule | None:
        if util.is_constraint(rule.head):
            ...
            for bodyElement in rule.body:
                if isinstance(bodyElement, ast.BodyAggregate | ast.BodySetAggregate):

                    left_guard = bodyElement.left
                    right_guard = bodyElement.right

                    left_guard, left_changed = self._normalize_guard(
                        left_guard, is_left=True
                    )
                    right_guard, right_changed = self._normalize_guard(
                        right_guard, is_left=False
                    )
                    left_guard, right_guard, collapse_changed = (
                        util.collapse_equal_bounds(self._lib, left_guard, right_guard)
                    )
                    if (
                        right_guard
                    ...
                    new_elem = bodyElement.update(
                        self._lib, **{"left": left_guard, "right": right_guard}
                    )
                    new_body.append(new_elem)
                else:
                    new_body.append(bodyElement)
