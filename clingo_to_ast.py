...
def sign_from(node_opt) -> ast.Sign:
    ...
    if node_opt.type == "sign":
        ...
        return (
            ast.Sign.Double
            if n >= 2
            else (ast.Sign.Single if n == 1 else ast.Sign.NoSign)
        )
    return ast.Sign.NoSign
...
def convert_term(lib: Library, src: bytes, node) -> ast.Term:
    ...
    if t in ("anonymous", "variable"):
        ...
        return ast.TermVariable(
            lib, location=loc, name=name, anonymous=(t == "anonymous")
        )

    # Bare identifier -> nullary function symbol
    if t in ("identifier", "negative_identifier"):
        return ast.TermFunction(
            lib, location=loc, name=text(src, node), pool=[], external=False
        )
    ...
    if t == "function":
        ...
        return ast.TermFunction(
            lib, location=loc, name=text(src, nm), pool=arg_tuples, external=False
        )
    ...
    if t == "external_function":
        ...
        arg_tuples = convert_arg_pool(lib, src, pool) if pool is not None else []
        return ast.TermFunction(
            lib, location=loc, name=text(src, nm), pool=arg_tuples, external=True
        )
    ...
    if t == "unary":
        ...
        uop = (
            ...
            if op_txt == "-"
                ...
                ast.UnaryOperator.Negation if op_txt == "~" else ast.UnaryOperator.Minus
            )
        )
        return ast.TermUnaryOperation(
            lib, location=loc, operator_type=uop, argument=rhs
        )

    if t == "abs":
        inner = next(
            (convert_term(lib, src, ch) for ch in named(node) if ch.type == "term"),
            None,
        )
        return ast.TermAbsolute(
            lib, location=loc, pool=[inner] if inner is not None else []
        )
    ...
    if t == "binary":
        ...
        return ast.TermBinaryOperation(
            lib, location=loc, operator_type=op_ty, left=lhs, right=rhs
        )
...
def collect_pool_groups(lib: Library, src: bytes, pool_node) -> List[List[ast.Term]]:
    ...
    for ch in named(pool_node):
        if ch.type in (
            ...
            cur = [
                convert_term(lib, src, tch) for tch in named(ch) if tch.type == "term"
            ]
            if cur:
                groups.append(cur)
...
def term_for_symbolic_atom(lib: Library, src: bytes, node) -> ast.Term:
    ...
    return ast.TermFunction(
        lib, location=ts_loc(lib, node), name=text(src, nm), pool=args, external=False
    )


def convert_comparison_literal(
    lib: Library, src: bytes, node, sign: ast.Sign, loc: Location
...
def _parse_upper_guard(
    lib: Library, src: bytes, upper_node
) -> Optional[ast.RightGuard]:
...
def _parse_condition_literals_from_element(
    lib: Library, src: bytes, el_node
) -> List[ast.Literal]:
...
def convert_body_aggregate(
    lib: Library, src: bytes, node, sign: ast.Sign
) -> ast.BodyAggregate:
    ...
    elems_parent = next(
        (c for c in named(node) if c.type == "body_aggregate_elements"), None
    )
    if elems_parent is not None:
        for el in named(elems_parent):
            ...
            if terms_node is not None:
                terms = [
                    convert_term(lib, src, t)
                    for t in named(terms_node)
                    if t.type == "term"
                ]
            else:
                terms = []
            ...
            elements.append(
                ast.BodyAggregateElement(
                    lib, location=el_loc, tuple=tup, condition=cond
                )
            )
    ...
    for ch in named(node):
        if ch.type == "literal_tuple":
            for g in named(ch):
                if g.type == "literal":
                    cond.append(
                        ast.BodySimpleLiteral(lib, literal=convert_literal(lib, src, g))
                    )
...
def convert_disjunction_head(lib: Library, src: bytes, node):
    ...
    for ch in named(node):
        if ch.type in (
            "conditional_literal",
            "conditional_literal_n",
            "conditional_literal_0",
        ):
            # head-conditional literal form
            hl = convert_conditional_literal_body(
                lib, src, ch
            )  # reuse body version to build literal/condition
            elems.append(
                ast.HeadConditionalLiteral(
                    ...
                    literal=hl.literal,
                    condition=[
                        ast.HeadSimpleLiteral(lib, literal=b.literal)
                        for b in hl.condition
                    ],
                )
            )
        elif ch.type == "literal":
            elems.append(
                ast.HeadSimpleLiteral(lib, literal=convert_literal(lib, src, ch))
            )
...
def convert_body_literal(lib: Library, src: bytes, node):
    ...
    if t == "_body_literal":
        sub = next(
            (
                c
                for c in named(node)
                if c.type in ("body_literal", "conditional_literal")
            ),
            None,
        )
    ...
    if t == "body_literal":
        ...
        if payload.type in ("symbolic_atom", "comparison", "boolean_constant"):
            lit = convert_simple_literal(
                lib, src, payload, sign_from(si), ts_loc(lib, node)
            )
...
def convert_statement(lib: Library, src: bytes, node):
    ...
    if t == "integrity_constraint":
        ...
        false_lit = ast.LiteralBoolean(
            lib, location=loc, sign=ast.Sign.NoSign, value=False
        )
...
def main():
    ap = argparse.ArgumentParser(
        description="Parse Clingo (clingo 6) with Tree-Sitter and build clingo.ast nodes."
    )
