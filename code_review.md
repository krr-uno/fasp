...
- **Data Clumps** — the `(context, auxiliary, library)` triple travels through ~9 functions; `library` is always `context.lib.library`. A small "lifter" object would absorb it (pre-existing pattern the diff extends). *Resolved (2026-07-13): the lifting machinery moved into a per-statement `_NegationLifter` class holding the context, derived library, and auxiliary collector; the entry points are thin wrappers, and all signatures shrank to essentially `(guards, node)`.*
- **Mysterious Name** — `tests/integration/base.py` carried over `self.elib` and the placeholder docstring `"""Assert transform equal."""` verbatim; the refactor was the moment to name them honestly. *Resolved (2026-07-13): `self.elib` → `self.library`; `assertTransformEqual` documents its dedent/strip/filter contract.*
